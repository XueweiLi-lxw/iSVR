...
SKAT_davies <- function(q,lambda,h = rep(1,length(lambda)),delta = rep(0,length(lambda)),sigma=0,lim=10000,acc=0.0001) {
  ...
  #Changed PACKAGE = "iSKAT" to SKAT because it was giving me an error and in the beginning it says copied without modification from SKAT 0.71
  #So it should not influence it at all
  out$res <- 1 - out$res
